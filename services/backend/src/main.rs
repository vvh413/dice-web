...
async fn main() -> std::io::Result<()> {
  ...
  HttpServer::new(|| {
    let frontend_path = std::env::var("FRONTEND_PATH").unwrap_or(DEFAULT_FRONTEND_PATH.to_string());
    App::new()
      ...
      .service(
        Files::new("/", frontend_path.clone())
          ...
          .default_handler(NamedFile::open(format!("{}/index.html", frontend_path)).unwrap()),
      )
